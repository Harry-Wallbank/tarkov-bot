...
- **`/metabuild weapon:M4A1`** — a greedy per-slot loadout optimizer, not just the default preset, **open to all members with no access restriction**. For every **required** mod slot on the weapon (recursing into whatever sub-slots the chosen mod itself exposes — e.g. a barrel's muzzle thread, then that muzzle device's own sub-slots), it picks whichever allowed, non-conflicting part gives the best combined ergonomics + recoil-reduction score. Optional slots (muzzle devices, sights, tactical rails, bipods, etc.) are otherwise left empty — **the one exception is a foregrip**, which is always added if the weapon has one, including whatever intermediate optional rail-mount slot is the only path to reach it (e.g. a KeyMod section on the handguard). Reports the full parts list, before/after ergonomics and recoil, total cost from traders, and a magazine pick balanced for capacity vs. reliability (not just "biggest mag wins"). See [`src/lib/metaBuildOptimizer.js`](src/lib/metaBuildOptimizer.js). This is **not** Tarkov.dev's own data or a community-curated meta ranking — it's this bot's own greedy heuristic over Tarkov.dev's item stats, which the embed footer says explicitly.
  - **Conflict-aware**: items carry `conflictingItems`/`conflictingCategories` data (e.g. one barrel can outright conflict with three specific handguards) — the optimizer checks this both directions before picking a part, so it won't assemble a build the game wouldn't actually let you attach.
